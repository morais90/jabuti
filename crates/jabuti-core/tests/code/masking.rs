use jabuti_core::code::masking::{self, MaskingKind};
use jabuti_core::model::{Detail, Rule, Severity};
...
fn masked(source: &str) -> Vec<(MaskingKind, String)> {
    masking::maskings(&parse_fixture(source))
        .into_iter()
...
fn masked_kotlin(source: &str) -> Vec<(MaskingKind, String)> {
    masking::maskings(&syntax::parse(source, &lang::KOTLIN).expect("fixture parses cleanly"))
        .into_iter()
        .map(|masking| (masking.kind, masking.construct))
        .collect()
}
...
fn a_finding_names_the_construct_and_what_it_costs() {
    let maskings = masking::maskings(&parse_fixture("fn f() {\n    let a = g().unwrap();\n}\n"));
...
fn the_finding_points_at_the_line_the_construct_is_on() {
    let source = "fn f() {\n    let value = builder()\n        .name(\"a\")\n        .build()\n        .unwrap();\n}\n";
    let maskings = masking::maskings(&parse_fixture(source));
